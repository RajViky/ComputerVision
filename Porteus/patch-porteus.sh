...
if [ -e $MODULE_NAME ]; then
    cp $MODULE_NAME $MODULE_NAME.backup
    rm $MODULE_NAME

    cp uvcvideo.ko $MODULE_NAME
fi

if [ -e $MODULE_NAME.xz ]; then
    cp $MODULE_NAME.xz $MODULE_NAME.xz.backup
    rm $MODULE_NAME.xz

    # compress
    xz uvcvideo.ko
    cp uvcvideo.ko.xz $MODULE_NAME
fi

if [ -e $MODULE_NAME.gz ]; then
    cp $MODULE_NAME.gz $MODULE_NAME.gz.backup
    rm $MODULE_NAME.gz

    # compress
    gzip uvcvideo.ko
    cp uvcvideo.ko.gz $MODULE_NAME.gz
fi
